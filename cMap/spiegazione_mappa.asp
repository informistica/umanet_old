...
if cint(Stato)=0 then
 'Definzione codice SQl della query per ricercare i nodi del paragrafo   
   if (Session("Admin")=True) or (Privato=0)  or (Nodi=1) then  'se vero visualizzo tutte i nodi del paragfrafo altrimenti solo quelle dello       studente loggato  
		...
		QueryCountSQL=costCountQuerySQL1 &_
		" HAVING Paragrafi.ID_Paragrafo='" & Codice_Test & "' and Nodi.Chi<>'?' and Nodi.Segnalata=0;   "
	else
		if condivisione = 1 then
		...
		"' ORDER BY Paragrafi.Posizione,Nodi.CodiceNodo;"
		 QueryCountSQL=costCountQuerySQL2 &_
		" HAVING Paragrafi.ID_Paragrafo='" & Codice_Test & "' and Nodi.Chi<>'?' and Nodi.Segnalata=0  and Nodi.Id_Stud='"& CodiceAllievo &";"   
		...
		"' ORDER BY Paragrafi.Posizione,Nodi.CodiceNodo;"
		QueryCountSQL=costCountQuerySQL2 &_
		" HAVING Paragrafi.ID_Paragrafo='" & Codice_Test & "' and Nodi.Chi<>'?' and Nodi.Segnalata=0  and Nodi.Id_Stud='"& Session("CodiceAllievo") &";"  
	...
	if super <> 1 then
			...
			if (Session("Admin")=True) or (Privato=0)  or (Nodi=1) then  'se vero visualizzo tutte i nodi del paragfrafo altrimenti solo quelle dello       studente loggato
				...
				" ORDER BY Paragrafi.Posizione,Nodi.CodiceNodo;"
				QueryCountSQL= costCountQuerySQL1 &_
				" HAVING Moduli.ID_Mod='" & Modulo & "' and Nodi.Chi<>'?' and Nodi.Segnalata=0  " &";" 
				...
				"' ORDER BY Paragrafi.Posizione,Nodi.CodiceNodo;"
				QueryCountSQL=costCountQuerySQL2 &_
				" HAVING Moduli.ID_Mod='" & Modulo & "' and Nodi.Chi<>'?' and Nodi.Segnalata=0   and Nodi.Id_Stud='"& CodiceAllievo &";"  
				...
				"' ORDER BY Paragrafi.Posizione,Nodi.CodiceNodo;"
				QueryCountSQL=costCountQuerySQL2 &_
				" HAVING Moduli.ID_Mod='" & Modulo & "' and Nodi.Chi<>'?' and Nodi.Segnalata=0   and Nodi.Id_Stud='"& Session("CodiceAllievo") &";" 
			...
			if (Session("Admin")=True) or (Privato=0)  or (Nodi=1) then  'se vero visualizzo tutte i nodi del paragfrafo altrimenti solo quelle dello       studente loggato
				...
				" ORDER BY Paragrafi.Posizione,Nodi.CodiceNodo;"
				QueryCountSQL= costCountQuerySQL1 &_
				" HAVING ("& Modulo1 & ") and Nodi.Chi<>'?' and Nodi.Segnalata=0;  "  
  ...
  Response.write QueryCountSQL &"<br>"
  
Set objFSO4 = CreateObject("Scripting.FileSystemObject")
url4=Server.MapPath(homesito & "/script/cMap/data/mappeutenti/debug.txt") 
url4=Replace(url4,"\","/")
'response.write(url2)
Set objCreatedFile4 = objFSO4.CreateTextFile(url4, True)
objCreatedFile4.WriteLine(QuerySQL)
objCreatedFile4.WriteLine(QueryCountSQL)
objCreatedFile4.Close
  
  
Set rsTabella = ConnessioneDB.Execute(QuerySQL)
cartella=rsTabella("Cartella")

Set rsTabellaCount = ConnessioneDB.Execute(QueryCountSQL)

numnodi=rsTabellaCount(0)

riga="""metrics"": {"
objCreatedFile.WriteLine(riga)
' DA SISTEMARE****** count nodi e connessioni
riga="""classCount"": 22, "
riga="""classCount"":"""&rsTabellaCount(0)&""","
objCreatedFile.WriteLine(riga)

...
call setInQuizOrderBy()
	...
	if strcomp(id_test,"0")<>0 then 'se anzichè tutti i paragrafi della vista ne prendo solo uno a scelta
	...
	Select Case id_app
		  Case 1
			 QuerySQL = "SELECT count(*)  FROM Leg_Domande WHERE (In_Quiz = '"&Quiz&"' or In_Quiz=-1) " &extraSql
		  Case 2
			 QuerySQL = "SELECT count(*)  FROM Cnv_Domande WHERE (In_Quiz = '"&Quiz&"' or In_Quiz=-1) " &extraSql
	...
	redim quesito(ndomande),ra(ndomande),rb(ndomande),rc(ndomande),rd(ndomande),re(ndomande), spiega(ndomande),tipo(ndomande)
		...
		Select Case id_app
		  Case 1
			 QuerySQL = "SELECT *  FROM Leg_Domande WHERE (In_Quiz = '"&Quiz&"' or In_Quiz=-1) " &extraSql
		  Case 2
			 QuerySQL = "SELECT *  FROM Cnv_Domande WHERE (In_Quiz = '"&Quiz&"' or In_Quiz=-1) " &extraSql
	...
	while ndate<numerodomande and i<=numerodomande 
	...
	if jsoncompleto="true" then
		 ...
		 response.write("""VF"&ndate&""": """&tipo(indice)&""", ""domanda"&ndate&""": """&ReplaceCar(quesito(indice))&""","  &_
	 """risposta"&ndate&".1"": """&ReplaceCar(ra(indice))&""", ""risposta"&ndate&".2"": """&ReplaceCar(rb(indice))&""", ""risposta"&ndate&".3"": """&ReplaceCar(rc(indice))&""", ""risposta"&ndate&".4"": """&ReplaceCar(rd(indice))&""", ""rispostaesatta"&ndate&""": """&ReplaceCar(re(indice))&"""," &_  
	  """spiegazione"&ndate&""": """&ReplaceCar(spiega(indice))&"""")
	 ...
	 wend
    response.write """domande"": """&ndate&""", ""tempo"": """&Tempo&""", ""titolo"": """&titolo&"""}"
	'response.write "}"

...
End sub
  ...
  QuerySQL="SELECT count(*) FROM Allievi where Email='"&Request.QueryString("email")&"';"
  set rsNumero=ConnessioneDB.Execute(QuerySQL)
	num = rsNumero(0)
	
	if num <> 0 and Request.QueryString("email") <> "" then
	
	  QuerySQL="SELECT CodiceAllievo,PasswordSHA256 FROM Allievi where Email='"&Request.QueryString("email")&"';"
	  ...
	  linkAvviso=dominio&homesito&"/script/cApp/cLogin/nuovapass.asp?CodiceAllievo="&rsTabella("CodiceAllievo")&"&hash="&rsTabella("PasswordSHA256")
	...
	" </tbody></table>"
	  ...
	  sFrom="Umanet Expo <info@umanetexpo.net>"
	  sTo=trim(Request.QueryString("email"))

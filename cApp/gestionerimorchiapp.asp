...
<head>
   ...
   <title>Gestione RimorchiApp</title>   
	...
	<div class="container-fluid" id="content">
	  ...
	  <div class="container-fluid">
				<div class="page-header">               
					<div class="pull-left">
						<h3> <i class="icon-comments"></i> Gestione RimorchiApp </h3> 
				...
				<div class="row-fluid">
				  <div class="span12">
				    <div class="box">
				      <div class="box-title">
				        <h3> <i class="icon-reorder"></i>  INVIA FRASE</h3>
			          </div>
				      <div class="box-content">
						...
						<div class="box-content">
							...
							<form action="cRimorchiApp/inseriscifrase.asp" method="post">
								<input type="text" class="input-small" value="Categoria" disabled>&nbsp;&nbsp;<!--<input name="categoria" id="categoria" type="text" class="input-xxlarge">-->
								
								<select id="categoria" name="categoria" class="input-xxlarge">
								...
								<input type="text" class="input-small" value="Frase" disabled>&nbsp;&nbsp;<input name="frase" id="frase" type="text" class="input-xxlarge">
								<br><br>
								<center><button class="btn" id="invia" type="button" onclick="controlla()">Invia Frase</button></center>
							</form>
			        ...
			        <div class="box">
				      <div class="box-title">
				        <h3> <i class="icon-reorder"></i>  ELENCO FRASI</h3>
			          </div>
				      <div class="box-content">
						...
						<div class="box-content">
							...
							do while not rsTabellaCategorie.EOF
								...
								QuerySQLC = "SELECT count(*) FROM RimorchiApp WHERE Categoria = '"&rsTabellaCategorie("Categoria")&"' and Approvata = '1';"
								set rsNumero = ConnessioneDB.Execute(QuerySQLC)
								
							%>
							
							<a href="gestionerimorchiapp.asp?mostra=<%=rsTabellaCategorie("Categoria")%>"><button class="btn"><b><%=rsTabellaCategorie("Categoria")%></b> (<%=rsNumero(0)%>)</button></a>&nbsp;
							...
							<% QuerySQLC = "SELECT count(*) FROM RimorchiApp WHERE 1=1 and Approvata = '1'"
								set rsNumero = ConnessioneDB.Execute(QuerySQLC)
								%>
							
							<a href="gestionerimorchiapp.asp"><button class="btn"><b>Tutte</b> (<%=rsNumero(0)%>)</button></a>
							...
							<table class="table table-hover table-nomargin">
								...
								if mostra <> "" then
									QuerySQL = "SELECT * FROM RimorchiApp WHERE Categoria = '"&mostra&"' and Approvata = '1' order by Categoria asc, Testo asc;"
								else
									QuerySQL = "SELECT * FROM RimorchiApp WHERE 1=1 and Approvata = '1' order by Categoria asc, Testo asc"
								end if
		...
		<form id="mod" action="cRimorchiApp/modificafrase.asp" method="post">
			<div id="modal-1" class="modal hide" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
				...
				<div class="modal-footer">
					...
					<button type="button" id="inviamodifica" class="btn btn-primary" onclick="controllamodifica()">Invia Modifica</button>
				</div>
			</div>
		</form>
		...
		function controlla(){
			...
			if(categoria == "" || frase == ""){
				...
				document.getElementById("invia").type="submit";
			}
		}
		...
		function modifica(id, frase, categoria, autore){
			...
			document.getElementById("mod").action="cRimorchiApp/modificafrase.asp?id="+id;
		}

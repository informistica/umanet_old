...
' se inserisco in più sessioni devo accodare le ultime Metafore dietro alle prime, quindi mi serve la posizione raggiunta per proseguire da li
...
Select case CodiceTest
   ...
   Case Cartella&"_U_2_5" 
	QuerySQL="Select count(*) from preNavigazione where Id_Paragrafo='"&Paragrafo&"';"
	QuerySQL2="Select max(Posizione) from preNavigazione where Id_Paragrafo='"&Paragrafo&"';"
...
if rsTabella(0)>0 then
 ...
 'response.write(QuerySQL& "???????????<br>" & "contPos="&contPos)
 cont=0
      ...
      if ((instr(Domanda,"tp://")<>0) or (instr(Domanda,"tps://")<>0)) Then
       ...
       Domanda =  ReplaceCar(Domanda)
		...
		'QuerySQL="INSERT INTO preDomanda (Id_Mod, Id_Paragrafo,Quesito,Eseguita) SELECT '" & Modulo & "','" & Paragrafo "','" & Domanda & "'," & 0 & "';"
				...
				Case Cartella&"_U_2_5" 
					  if Scadenza <>"" and not (strcomp(Scadenza,"gg/mm/aaaa")=0) then
				      QuerySQL="  INSERT INTO preNavigazione (Id_Mod, Id_Paragrafo,Quesito,Eseguita,Posizione,Scadenza,Img,Files,Id_Sottoparagrafo)  SELECT '" & Modulo & "','" & Paragrafo & "', '" & Domanda & "'," & 0 & "," & contPos+k & ",'" & Scadenza & "'," & img & "," & cFile & ",'" & CodiceSottopar & "';"
				      else
				 	  QuerySQL="  INSERT INTO preNavigazione (Id_Mod, Id_Paragrafo,Quesito,Eseguita,Posizione,Scadenza,Img,Files,Id_Sottoparagrafo)  SELECT '" & Modulo & "','" & Paragrafo & "', '" & Domanda & "'," & 0 & "," & contPos+k & ",'" & fine_anno & "'," & img & "," & cFile & ",'" & CodiceSottopar & "';"
				     end if
				Case Cartella&"_U_2_8"	
		...
		'        				url1="C:\Inetpub\umanetroot\anno_2012-2013\logpreMetafore.txt"
...
<%
   ...
   %>
		  ...
		  <h5><a href="2inserisci_premetafore.asp?Capitolo=<%=Capitolo%>&TitoloParagrafo=<%=TitoloParagrafo%>&Paragrafo=<%=Paragrafo%>&Modulo=<%=Modulo%>&CodiceSottopar=<%=CodiceSottopar%>">Continua ...</a></h5>
		<p>&nbsp;</p>

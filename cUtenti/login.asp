...
<%  
   ...
   QuerySQL="SELECT Cognome,Nome,Password,In_Quiz,Id_Classe,Classe,CodiceAllievo,Stile"&_
	 		  " FROM [dbo].[Allievi]" &_ 
			  " WHERE CodiceAllievo='" & CodiceAllievo& "' and Password = '" & PwdAllievo & "';"
	   Set rsTabella = ConnessioneDB.Execute(QuerySQL)
	     if not rsTabella.eof then
	   PwdAllievoDB=rsTabella("Password")
	   end if
	...
	' ho trovato lo studente adesso devo stabilire a quale id_classe appartiene quindi eseguo query su stud_as_classe (precedentemente popolata) con id_as e id_stud
		...
		'  DataClaq2= left(now(),10)
		 ...
		 Response.Cookies("Dati")("DBDesideri")= Session("DBDesideri") 		
		  ...
		  response.write("Cognome="&session("Cognome")&"Nome="&session("Nome")&"stile="&session("stile")&"&id_classe="&id_classe&"&classe="&classe&"&cod="&CodiceAllievo&"&DataClaq2="&DataClaq2&"&DataClaq="& DataClaq)

...
if NUMTEST="" then%>
<form method="POST" class="form-horizontal" action="3correggi_test_new_test.asp?Stato=<%=Stato%>&Cartella=<%=Cartella%>&CodiceTest=<%=CodiceTest%>&Capitolo=<%=Capitolo%>&Paragrafo=<%=Paragrafo%>&Modulo=<%=Modulo%>&CodiceSottopar=<%=CodiceSottopar%>&Sottoparagrafo=<%=Sottoparagrafo%>&vf=<%=vf%>&rm=<%=rm%>">
...
stringQuery2= "' order by CodiceDomanda asc;"
dim vetQuerySQL(4),numDom(4)
...
if (Stato=0) then 
     if CodiceSottopar<>"" then	
			...
			for i=0 to 3 
			vetQuerySQL(i)="SELECT count(*) " &_
             "FROM DomandeQuiz WHERE " &_
            stringQuery& " Id_Arg='" & CodiceTest & "'  and Id_Sottoparagrafo='" & CodiceSottopar & "' and In_Quiz="&i+1
			next 
			...
			vetQuerySQL(i)="SELECT count(*) " &_
             "FROM DomandeQuiz WHERE " &_
            stringQuery& " Id_Arg='" & CodiceTest & "' and In_Quiz="&i+1
			next 
    end if
   ...
   QuerySQL="SELECT *" &_
			...
			vetQuerySQL(i)="SELECT count(*) " &_
             "FROM DomandeQuiz WHERE " &_
             stringQuery&" Id_Mod='" & Modulo & "' and In_Quiz="&i+1
			next 
end if  
   
   
   ' calcolo il numero di domande per ogni batteria
          for i=0 to 3 
		    NumDom(i)=0	
			' response.write(vetQuerySQL(i)&"<br>")
			 Set rsTabella = ConnessioneDB.Execute(vetQuerySQL(i))
			 if not rsTabella.eof then
				NumDom(i)=rsTabella(0)
			 end if
				response.write("- Batteria " & i+1 & " ha " & NumDom(i)& " domande<br>")    
			next 
   response.write("<br>")
...
<% Else %>
  <FORM class="form-horizontal" METHOD="POST" ACTION="3correggi_test1_new.asp?Stato=<%=Stato%>&Modulo=<%=Modulo%>&CodiceTest=<%=CodiceTest%>&NUMTEST=<%=NUMTEST%>&CodiceSottopar=<%=CodiceSottopar%>&Sottoparagrafo=<%=Sottoparagrafo%>&vf=<%=vf%>&rm=<%=rm%>">
 <div class="control-group">
		 
  <%i=1 
   numDomInQuiz=0 'inizializza la variabile i (contatore delle domande)%>

 <%'response.write("<b>Numero Batteria - Quesito - Segnalata</b>")%>
  <table class="table table-hover table-nomargin table-bordered table-striped">
	<tr><th>Numero Batteria</th><th>Quesito</th><th>Segnalata</th></tr>
  ...
  'response.write("<br>")
   if (rsTabella.Fields("Tipo")=1 ) then ' inserisco domanda plus leggendola dal file  altrimenti domanda normale %>
    <tr><td><B> <%=i & ") "%><%=rsTabella.Fields("Quesito")%></B> </td>
	 <td><textarea rows="6" name="S1" value="ciao" cols="116"><%=Response.write(domandaplus())%> </textarea><br></td></tr>
  <%else%>
     
      <tr><td>  
     <INPUT TYPE="text" class="input-mini"   NAME="In_Quiz_<%=i%>" VALUE="<%=rsTabella.Fields("In_Quiz")%>"></td><td><B>  <%=rsTabella.Fields("Quesito")%></B></td><td> <INPUT TYPE="text" class="input-mini"   NAME="segnalata<%=i%>" VALUE="<%=rsTabella.Fields("Segnalata")%>">
		 </td></tr>
   ...
   <P>
      <INPUT TYPE="SUBMIT" NAME="submit" VALUE="Totale <%=i%>"> <!-- crea il bottone per inviare le riposte alla pagina che calcola il risultato -->
   </P>

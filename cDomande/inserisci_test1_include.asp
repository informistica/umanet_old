 ...
 'response.write("ID_Predomanda="&ID_Predomanda)
   if len(RE)=0 then ' serve
     RE="0"
   end if
 ...
 if (errore=0) then
		 ' inserisci la domanda
			...
			if Valutato=1 then
				 ...
				 if (DVAbilitato=1)and (len(Spiegazione)>350)   then
					    'voto=2  ' neutralizzo il bonus random
						Segnalata=0
				 end if 
	   ...
	   ' if (strcomp(Tipo,"1")=0) then ' se la domanda è plus metto nullo il campo che poi aggiorno con l'url del file di testo che contiene la domanda dopo
			 ...
			 'FORSE possa fare una query unica 
			  QuerySQL="INSERT INTO Domande (Quesito, Risposta1, Risposta2,Risposta3,Risposta4,RispostaEsatta,Id_Stud,Id_Arg,Id_Mod,Data,Voto,Cartella,Tipo,In_Quiz,Multiple,ID_Predomanda,Img,Ora,Id_Sottoparagrafo,Lingua) SELECT '"& Titolo & "','"& R1 & "', '" & R2 & "','" & R3 & "','" & R4 & "','" & RE & "','" & CodiceAllievo & "','" & CodiceTest & "','" & Modulo & "','"  & DataTest & "'," & Voto & ",'" & Cartella & "','" & Tipo & "','" & In_Quiz_Stud &"','" & Multiple &"'," & ID_Predomanda &"," & Img &",'" & FormatDateTime(now, 4)  & "','" & "','" & lingua & "';"
	...
	'	prelava ID dell'ultimo record inserito
		...
		'if (strcomp(Tipo,"1")=0)  then ' se la domanda è plus aggiorno con l'url del file di testo che contiene la domanda 
		  url4=Server.MapPath(homesito)& "/Db"&Session("DB")&"/Materie/"&Session("ID_Materia")& "/" & CARTA &"/" &Modulo&"_Domande/"&Modulo&"_"&Paragrafo&"_"&ID&".txt"

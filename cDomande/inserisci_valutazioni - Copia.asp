...
<fieldset><legend> 
	...
	<br><a href="#modal-1" onClick="pippo();" data-toggle="modal"><i style="text-decoration:none" class="icon-pencil" title="Modifica domanda"></i></a>&nbsp;&nbsp;
...
<%end if ' if not rsTabellaNew.eof %>			 
	   ...
	   <form id="mod"  method="post">
			<div id="modal-1" class="modal hide" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none; overflow: scroll;">
				...
				<div class="modal-body">
					...
					<input  name="url" id="url" type="hidden"><input  name="codice" id="codice" type="hidden">
		...
		function modifica(quesito,a,b,c,d,e,spiegazione,codice,url,segnalata){
		// alert(url);
		 //alert(codice);
		...
		function controllamodifica(){
			var titolo = document.getElementById("txtQuesito").value.trim();
			...
			}else
			{
				...
				$.ajax({
					  method: "POST",
					  url: "inserisci_aggiornamento_domanda.asp?id="+codice+"&quesito="+titolo+"&r1="+a+"&r2="+b+"&r3="+c+"&r4="+d+"&re="+e+"&url="+url+"&spiegazione="+spiegazione+"&segnalata="+segnalata,
					  dataType: "html",
					  data: {  }
					}) /* .ajax */
					 .done(function( ans ) {

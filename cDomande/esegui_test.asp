...
' genera un numero casuale per scegliere quale quiz e quale ordinamento per le domande 
		   ...
		   " WHERE Domande.Id_Arg='" & CodiceTest & "'  and Domande.Id_Sottoparagrafo='" & CodiceSottopar & "' and Domande.Multiple=0  and Domande.VF=0;"
			...
			QuerySQL="SELECT MAX([In_Quiz]) AS [Num_Quiz] "&_
			 ...
			 ' Set objFSO = CreateObject("Scripting.FileSystemObject")  
			' url="C:\Inetpub\umanetroot\expo2015Server\logSetInquiz.txt"
			...
			if Num_Quiz=-1 then
			   ...
			   cont=0
			    do 
					cont=cont+1
					rand=rnd()
				loop until (clng(left((rand*5),1))>0) and (clng(left((rand*5),1))<=7) and (cont<100)
				if cont=100 then
					orderby=1
				  else
				...
				if Num_Quiz>0 then
					...
					do 
						cont=cont+1
						rand=rnd()
					loop until (clng(left((rand*5),1))>0) and (clng(left((rand*5),1))<=Num_Quiz) and (cont<100)
					if cont=100 then
						Quiz=1
					  else
					Quiz=left((rand*5),1)
				...
				<div class="page-header">               
                   ...
                   <!--     <a title="Condividi link alla pagina" href="#" onClick="javascript:PopUpWindow(600,400);return false;"><i class="glyphicon-share_alt"> </i> <small>Condividi</small> </a>  -->
					...
					<div class="pull-right">
                     <!-- se mi interessa devo includere include pull_right.asp-->	 
                    </div>
				</div>
...
if (Stato=0) then 
	...
	if CodiceSottopar<>"" then
		QuerySQL="SELECT * "&_
		" FROM Paragrafi INNER JOIN Domande ON Paragrafi.ID_Paragrafo = Domande.Id_Arg" &_
		" WHERE Domande.Id_Arg='" & CodiceTest & "' and  Domande.Segnalata=0 and Domande.Id_Sottoparagrafo='" & CodiceSottopar & "' and Domande.Multiple=0 and Domande.VF=0 and Lingua='"&Lingua&"';"
	else
		QuerySQL="SELECT * "&_
		" FROM Paragrafi INNER JOIN Domande ON Paragrafi.ID_Paragrafo = Domande.Id_Arg" &_
		" WHERE Domande.Id_Arg='" & CodiceTest & "' and  Domande.Segnalata=0  and Domande.Multiple=0  and Domande.VF=0 and Lingua='"&Lingua&"';"
	end if
else 
...
'		   
			 ...
			 ' Set objFSO = CreateObject("Scripting.FileSystemObject")  
			' url="C:\Inetpub\umanetroot\expo2015Server\logEseguiQuizz.txt"
						' Set objCreatedFile = objFSO.CreateTextFile(url, True)
						' objCreatedFile.WriteLine(QuerySQL & "orderby="&orderby)
						' objCreatedFile.Close 
			'response.write("<br>ciao numquiz"&QuerySQL)			
			Set rsTabella = ConnessioneDB.Execute(QuerySQL) 
		...
		if (Stato=0) then   
		  if CodiceSottopar<>"" then
		   ...
		   " WHERE Domande.Id_Arg='" & CodiceTest & "' and  Domande.Segnalata=0  and Domande.Id_Sottoparagrafo='" & CodiceSottopar & "' and  Domande.Multiple=0 and Lingua='"&Lingua&"  and Domande.VF=0 AND (Domande.In_Quiz=" &Quiz & "  or Domande.In_Quiz=-1) order by Domande." & order(orderby)& " asc;"
		  
		   else
    QuerySQL="SELECT Domande.*, Paragrafi.Titolo,Allievi.Cognome,Allievi.Nome " &_
		   " FROM Allievi INNER JOIN (Paragrafi INNER JOIN Domande ON Paragrafi.ID_Paragrafo = Domande.Id_Arg) ON Allievi.CodiceAllievo = Domande.Id_Stud " &_
		   " WHERE Domande.Id_Arg='" & CodiceTest & "'and  Domande.Segnalata=0  and  Domande.Multiple=0  and Domande.VF=0 and Lingua='"&Lingua&"' AND (Domande.In_Quiz=" &Quiz & "  or Domande.In_Quiz=-1) order by Domande." & order(orderby)& " asc;"
		...
		'Definzione codice SQl della query per sapere quanti quiz ci sono
		 QuerySQL="SELECT MAX([In_Quiz]) AS [Num_Quiz] "&_
		  " FROM Paragrafi INNER JOIN Domande ON Paragrafi.ID_Paragrafo = Domande.Id_Arg" &_
		   " WHERE Domande.Id_Mod='" & Modulo & "'and  Domande.Segnalata=0  and Domande.Multiple=0  and Domande.VF=0 ;"
			Set rsTabella = ConnessioneDB.Execute(QuerySQL) 
			call setInQuizOrderBy()
		...
		QuerySQL="SELECT Domande.*, Paragrafi.Titolo,Allievi.Cognome,Allievi.Nome " &_
		   " FROM Allievi INNER JOIN (Paragrafi INNER JOIN Domande ON Paragrafi.ID_Paragrafo = Domande.Id_Arg) ON Allievi.CodiceAllievo = Domande.Id_Stud " &_
		   " WHERE Domande.Id_Mod='" & Modulo & "' and  Domande.Segnalata=0  and Domande.Multiple=0  and Domande.VF=0 and Lingua='"&Lingua&"' " &stringaQuery  &" order by Domande." & order(orderby)& " asc;"

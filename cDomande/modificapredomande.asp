...
<head>
   ...
   <title>Modifica predomande</title>   
	...
	<div class="container-fluid" id="content">
	  ...
	  <div class="container-fluid">
				<div class="page-header">               
					<div class="pull-left">
						<h3> <i class="icon-question-sign"></i> <%=Capitolo%>: <%=Paragrafo%></h3> 
				...
				<div class="row-fluid">
				  <div class="span12">
				    <div class="box">
				      <div class="box-title">
				        <h3> <i class=" icon-lock"   data-placement="right" title="Valute le domande guidate" rel="tooltip" ></i>  DOMANDE DISPONIBILI</h3>
			          </div>
...
if Elimina<>"" then
 ...
 QuerySQL="Delete * " &_
"FROM preDomande WHERE preFrasi.ID_Predomanda=" & ID & ";" 
Set rsTabella = ConnessioneDB.Execute(QuerySQL) 
QuerySQL="Delete * " &_
"FROM Domande WHERE ID_Predomanda=" & ID & ";" 
Set rsTabella = ConnessioneDB.Execute(QuerySQL) 
...
"FROM preDomande WHERE preDomande.Id_Paragrafo='" & CodiceTest & "'" 
Set rsTabella = ConnessioneDB.Execute(QuerySQL) 
NumRec=rsTabella(0)
Numero=clng(NumRec)
Dim dom()
Redim dom(Numero)
                                      ...
                                      ' 0			  1					2				3					4	
 QuerySQL="SELECT * " &_
"FROM preDomande WHERE preDomande.Id_Paragrafo='" & CodiceTest & "' order by Posizione" 
Set rsTabella = ConnessioneDB.Execute(QuerySQL)
...
<%else%>
<form method="POST" name="dati" class="form-vertical" action="../modificapredomande.asp?BoxApro=<%=BoxApro%>&Modifica=1&Id_Stud=<%=Id_Stud%>&CodiceTest=<%=CodiceTest%>&NumRec=<%=NumRec%>&Capitolo=<%=Capitolo%>&Paragrafo=<%=Paragrafo%>">	
...
<%do while not rsTabella.eof
	...
	dom(i)=rsTabella.fields("Quesito")
	 %>	
			<input type="text" class="hidden" value="<%=rsTabella.fields("id_Predomanda")%>" name="txtIdFrase<%=i%>" size="3" > 
			<fieldset><legend><%=i+1%> Domanda	</legend>
           							 <div class="control-group">
										
										<div class="controls">
											<input type="text" value="<%=rsTabella.fields("Quesito")%>" name="txtFrase<%=i%>" class="input-xxlarge"> &nbsp;&nbsp; <img src="../../img/elimina.jpg" width="16" height="16"  onClick="elimina(<%=rsTabella.fields("id_Predomanda")%>);" title="Elimina"><br>
										</div>
									</div>
            ...
            <div class="control-group">
                                        ...
                                        &nbsp;&nbsp;&nbsp;
                                         <span title="Richiede caricamento file ?"><b>File</b></span> 
                                             ...
                                             <% if (rsTabella.fields("File")=1)  then  %>
   ...
   'TestoDomandaPlus=Request.Form("TestoDomandaPlus")
      if len(Id_Stud)>0 then ' aggiungo eccezioni
		   ...
		   if DateDiff("D", Date(), Scadenza)>=0 then
			   ...
			   QuerySQL="SELECT count(*) FROM Eccezioni_Domande  WHERE  Id_Stud='"&Id_Stud&"' and Id_Predomanda='"&ID&"';" 
			   Set rsTabella = ConnessioneDB.Execute(QuerySQL)
			   ris=rsTabella(0)
			   
				if ris=0 then
					QuerySQL="INSERT INTO Eccezioni_Domande (Id_Predomanda,Id_Stud,Scadenza) SELECT '" & ID  & "','" & Id_Stud & "','" & Scadenza & "';"
					ConnessioneDB.Execute(QuerySQL)
				else
					QuerySQL = "UPDATE Eccezioni_Domande SET Scadenza = '" &Scadenza&"' WHERE Id_Predomanda = '"&ID&"' and Id_Stud = '"&Id_Stud&"';"
					ConnessioneDB.Execute(QuerySQL)
				end if
		   end if
	  
	  else
	      QuerySQL ="UPDATE preDomande SET Quesito = '" & Quesito & "', Scadenza = '" & Scadenza & "', Img = " & Img & ", Posizione = " & Pos& ", File = " & cFile &" WHERE Id_Predomanda =" &ID&";"
		  ConnessioneDB.Execute(QuerySQL) 
	  end if	 
	  ' response.Write(QuerySQL)
...
<% end if %>				 	 
 ...
 <script language="javascript" type="text/javascript"> 
function elimina(id) {
    document.dati.action = "modificapredomande.asp?ID=" + id +"&Elimina=1";

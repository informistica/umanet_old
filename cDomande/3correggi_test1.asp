...
if (Stato=0) then 
  ...
  Do While not(rsTabella.EOF) 
     ...
     CASE "0"
	    Escludi=0 ' se è selezionato  input box a sinistra del quesito allora abilito la domanda
     CASE "5" 
     	Escludi=1 '  se è selezionato  input box a destra del quesito allora disabilito la domanda
     CASE ELSE
	    Escludi=2
	 END SELECT  
     
    ' if (Escludi=0) and (rsTabella.fields("In_Quiz")<> clng(NUMTEST)) then ' se non era inclusa la includo 
         if (Escludi=0)  then ' se non era inclusa la includo 
     
		QuerySQL ="UPDATE Domande SET In_Quiz = " & clng(NUMTEST) & " WHERE CodiceDomanda =" &rsTabella(0) &";"	
		ConnessioneDB.Execute QuerySQL 
		incluse=incluse+1
	    response.write(QuerySQL & "<br>")
     else
		' if (Escludi=1) and (rsTabella.fields("In_Quiz")= clng(NUMTEST)) then ' se  inclusa la escludo 
			if (Escludi=1)  then ' se  inclusa la escludo 
		
			QuerySQL ="UPDATE Domande SET In_Quiz = 0 WHERE CodiceDomanda =" &rsTabella(0) &";"	
			ConnessioneDB.Execute QuerySQL 
			escluse=escluse+1
			response.write(QuerySQL & "<br>")
		 end if 
 ...
 %>
		    ...
		    <h5> <span class="alert-error"> Escluse <%=escluse%> domande</span></h5>
			<h5> <span class="alert-info">  Incluse <%=incluse%> domande </span></h5>
			<h5> <span class="alert-success"> Netto  <%=incluse-escluse%> domande</span></h5>

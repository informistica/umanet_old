...
'						objCreatedFile.Close 
		...
		<p align="center"><b><%Response.write ("N."&Quiz) %></b>
		 ...
		 <%
		   ...
		   " WHERE Domande.Id_Arg='" & CodiceTest & "'  and Domande.Id_Sottoparagrafo='" & CodiceSottopar & "'  and  Domande.Multiple=0 and Domande.VF=1 AND (Domande.In_Quiz=" &Quiz & "  or Domande.In_Quiz=-1) order by Domande." & order(orderby)& " asc;"
		
		  
		   else
 QuerySQL="SELECT Domande.*, Paragrafi.Titolo,Allievi.Cognome,Allievi.Nome " &_
		   " FROM Allievi INNER JOIN (Paragrafi INNER JOIN Domande ON Paragrafi.ID_Paragrafo = Domande.Id_Arg) ON Allievi.CodiceAllievo = Domande.Id_Stud " &_
		   " WHERE Domande.Id_Arg='" & CodiceTest & "' and  Domande.Multiple=0 and Domande.VF=1 AND (Domande.In_Quiz=" &Quiz & "  or Domande.In_Quiz=-1) order by Domande." & order(orderby)& " asc;"
		...
		QuerySQL="SELECT Domande.*, Paragrafi.Titolo,Allievi.Cognome,Allievi.Nome " &_
		   " FROM Allievi INNER JOIN (Paragrafi INNER JOIN Domande ON Paragrafi.ID_Paragrafo = Domande.Id_Arg) ON Allievi.CodiceAllievo = Domande.Id_Stud " &_
		   " WHERE Domande.Id_Mod='" & Modulo & "' and Domande.Multiple=0 and Domande.VF=1 AND (Domande.In_Quiz=" &Quiz & " or Domande.In_Quiz=-1) order by Domande." & order(orderby)& " asc;"
		 '  response.write(QuerySQL)
        ...
        <% else %>    
		  ...
		  ' esegue un ciclo e ad ogni iterazione crea un quiz (con 4 valori possibili) avente per nome il numero contenuto nella variabile i 

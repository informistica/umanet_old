...
<BODY> 
 ...
 Dim ConnessioneDB, rsTabella, QuerySQL, CodiceTest, CodiceAllievo,PwdAllievo, CodiceCorso, i,StringaConnessione,stato,CodiceDomanda,RE,appo
 Dim R()
...
<p align="center"><b><font face="Verdana" size="4" color=#FF0000>MESCOLA TEST </font></b></p>
...
if (Stato=0) then 
 ...
 ' mi serve anche il titolo del paragrafo per ricostruire il nome del file che contine la domanda plus
   QuerySQL="SELECT Domande1.*, Paragrafi.Titolo " &_
   " FROM Paragrafi INNER JOIN Domande1 ON Paragrafi.ID_Paragrafo = Domande1.Id_Arg" &_
...
<% Else %>
  ...
  Do until rsTabella.EOF   ' esegue un ciclo e ad ogni iterazione crea un quiz (con 4 valori possibili) avente per nome il numero contenuto nella variabile i 
		...
		for i=1 to 4 ' carico nel vettore le 4 risposte
		   ...
		   R(i)=rsTabella.Fields(2+i)
		next 
		CodiceDomanda=rsTabella.Fields(0)
		RE=rsTabella.Fields(7) ' prelevo il numero della risposta esatta
		...
		do ' genero il numero casuale per sapere dove spostare la domanda esatta
		  rand1=left((rnd()*5),1)
		loop until (rand1>0) and (rand1<5) and R(rand1)<>"Nessuna delle precedenti"
		'rand1=left((rand*5),1)
        ...
        next
 QuerySQL ="UPDATE Domande1 SET Risposta1 = '" &R(1)& "', Risposta2='" &R(2)&"', Risposta3 ='" &R(3)& "',Risposta4 = '" &R(4)& "', RispostaEsatta = '" &RE& "'  WHERE Domande1.CodiceDomanda= "&CodiceDomanda & ";"

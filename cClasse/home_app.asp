...
'Response.AddHeader "Refresh", "500" sostituito dal javascript in fondo
...
<!--	<meta charset="utf-8">-->
    ...
    <title>Libro <% if umanet<>"" then response.write("U")%></title>
    <%
...
%>
 ...
 ValidaQuiz=rsTabellaSetting("ValidaQuiz")
	  ...
	  QuerySQL="SELECT Id_Classe, Titolo, TitPar, ID_Mod, ID_Paragrafo,Cartella,URL,URL_OL,Classe,URL_L,URL_O,Posizione from MODULI_UMANET1  where Id_Classe='"&id_classe&"' and Visibile=1 order by PosMod, PosPar ;"
					'
                end if
...
<!-- #include file = "../include/navigation_small.asp" -->
				...
				<div class="breadcrumbs">
					<ul>
						...
						<li>
							<a href="#"><b>Libro <% if umanet<>"" then response.write("uwww")%></b></a>
						...
						<div class="box">
							<div class="box-title">
								<h3>
									...
									Libro <% if umanet<>"" then response.write("umanet")%> classe <%=left(Classe,1+len(Classe)-instr(Classe,"$"))%>
									<% if visibili="" then %>
									...
									<%end if%>
									<%if session("admin")=true then%>
									<i class="icon-edit" aria-hidden="true" title="Genera piano di lavoro con home_app_piano_lavoro.asp" alt="Genera piano di lavoro con home_app_piano_lavoro.asp"></i>
  ...
  <div class="accordion-heading">
   ...
   <section id="collapse">
    ...
    <%if (instr(rsTabella("URL_OL"),"https")<>0) or (instr(rsTabella("URL_OL"),"http")<>0) then %>
             <a  title="Risorse introduttive" href="<%=rsTabella("URL_OL")%>" target="_blank"><i class="icon-cloud"></i>&nbsp;&nbsp;-&nbsp; </a>
		<%else%>
    ...
    <%end if %>
         <% if ValidaQuiz=1 then%>
          ...
          &nbsp;&nbsp;- <%end if%>
			...
			<% if Session("Admin") = true then %>
			&nbsp;&nbsp;&nbsp;- 
			<a data-placement="right"   rel="tooltip" title="Valuta Frasi" href="../cFrasi/2scegli_valutazioni_frasi.asp?soloimg=1&BoxApro=<%=iddiv%>&Cartella=<%=rsTabella(5)%>&CodiceTest=<%=rsTabella(4)%>&Capitolo=<%=rsTabella(1)%>&Paragrafo=<%=rsTabella(4)%>&TitoloParagrafo=<%=rsTabella(2)%>&Modulo=<%=rsTabella(3)%>&tutto=1"><%=response.write(" (=")%></a>
				&nbsp; <i class="icon-reply"></i>
            <a data-placement="right"   rel="tooltip" title="Modifica Frasi" href="../cFrasi/2modificaprefrase.asp?modifica_scadenze_classe=<%=modifica_scadenze_classe%>&BoxApro=<%=iddiv%>&Id_Stud=<%=Id_Stud%>&Capitolo=<%=rsTabella(1)%>&Modulo=<%=rsTabella(3)%>&tutto=1&Cartella=<%=cartella%>"><%=response.write(" m) ")%></a>&nbsp;
            ...
            do while not rsTabellaSottopar.eof
				 ...
				 &nbsp;&nbsp; &nbsp; &nbsp;&nbsp; &nbsp; &nbsp;&nbsp;<a rel="popover" data-trigger="hover" data-content="Crea nodo della rete concettuale"  Title="Compila Nodo (N)" target=blank href="../cNodi/1compilaprenodo.asp?Cartella=<%=rsTabella(5)%>&Capitolo=<%=rsTabella(1)%>&Paragrafo=<%=rsTabella(2)%>&Modulo=<%=rsTabella(3)%>&CodiceTest=<%=rsTabella(4)%>&prenodo=1&Sottoparagrafo=<%=rsTabellaSottopar("Titolo")%>&CodiceSottopar=<%=rsTabellaSottopar("ID_Sottoparagrafo")%>" ><span style="text-transform: uppercase;"> <i class="glyphicon-snowflake"></i></span></a>&nbsp;&nbsp;&nbsp;&nbsp;
                   ...
                   <a data-placement="right"   rel="tooltip" title="Modifica Risorse"  href="../cAdmin/modificamodulo.asp?ID_Mod=<%=rsTabella.fields("ID_Mod")%>&Classe=<%=Classe%>&Id_Classe=<%=Id_Classe%>&Sottoparagrafo=<%=rsTabellaSottopar("Titolo")%>&CodiceSottopar=<%=rsTabellaSottopar("ID_Sottoparagrafo")%>">(m)</a>
                     <% end if%>
                     <% if rsTabellaSottopar("URLDOC")<>"" then %>
                    
                   <a data-placement="right"   rel="tooltip" title="Risorse docente" target="_blank"  href="<%=rsTabellaSottopar("URLDOC")%>"> (!)</a>
                     <% end if%>
                      &nbsp;&nbsp;&nbsp;
                    ...
                    <a data-placement="right"   rel="tooltip" title="Modifica Frasi" href="../cFrasi/2modificaprefrase.asp?modifica_scadenze_classe=<%=modifica_scadenze_classe%>&BoxApro=<%=iddiv%>&Id_Stud=<%=Id_Stud%>&CodiceTest=<%=rsTabella(4)%>&Capitolo=<%=rsTabella(1)%>&Paragrafo=<%=rsTabella(2)%>&Sottoparagrafo=<%=rsTabellaSottopar("Titolo")%>&CodiceSottopar=<%=rsTabellaSottopar("ID_Sottoparagrafo")%>&Modulo=<%=rsTabella(3)%>&Cartella=<%=cartella%>"><%=response.write(" m ")%>)</a>&nbsp;
...
<a href="#" data-rel="tooltip" data-placement="bottom" title="Tooltip on bottom">
                   ...
                   <a data-placement="right"   rel="tooltip" title="Modifica Risorse"  href="../cAdmin/modificamodulo.asp?ID_Mod=<%=rsTabella.fields("ID_Mod")%>&Classe=<%=Classe%>&Id_Classe=<%=Id_Classe%>&divid=<%=divid%>">(m)</a>
                    ...
                    <a data-placement="right"   rel="tooltip" title="Modifica Frasi" href="../cFrasi/2modificaprefrase.asp?modifica_scadenze_classe=<%=modifica_scadenze_classe%>&BoxApro=<%=iddiv%>&Id_Stud=<%=Id_Stud%>&CodiceTest=<%=rsTabella(4)%>&Capitolo=<%=rsTabella(1)%>&Paragrafo=<%=rsTabella(2)%>&Modulo=<%=rsTabella(3)%>&Cartella=<%=cartella%>"><%=response.write(" m ")%>)</a>&nbsp;

<% 
 ...
 <%QuerySQL="SELECT * FROM  Elenco_Metafore_Topolino WHERE (Data>= CONVERT(DATETIME,'" &DataClaq  &"', 104))" &_
	 " AND (Data<= CONVERT(DATETIME,'" &CDATE(DataClaq2) &"', 104))"&_
	  " AND ID_Paragrafo='"&Cartella&"_U_2_3"&"';" 
	  %>
           
								<% Case Cartella&"_U_2_5" 'Navigazione%>
  <%QuerySQL="SELECT * FROM  Elenco_Metafore_Mavigazione WHERE (Data>= CONVERT(DATETIME,'" &DataClaq  &"', 104))" &_
	 " AND (Data<= CONVERT(DATETIME,'" &CDATE(DataClaq2) &"', 104))"&_
	  " AND ID_Paragrafo='"&Cartella&"_U_2_5"&"';" 
	   %>
							  		<% Case Cartella&"_U_2_8" 'ClientServer%>
                                     <%QuerySQL="SELECT * FROM  Elenco_Metafore_Desideri " &_

...
<div class="box box-color box-bordered">
                                            <div class="box-title">
                                                <h3>
                                                    ...
                                                    Lavagna: Discussioni aperte (<%=num_post_totali%>) + Commenti(<%=num_messaggi%>) = Punti(<%=num_post_totali_punti+num_messaggi_punti%>)
                                                </h3>
                                            </div> 
...
<%If rsTabellaLavagna.BOF=True And rsTabellaLavagna.EOF=True Then %>
    ...
    <thead>             
			  <tr><th colspan="5" align="center">  Non ci sono attivit&agrave; nella lavagna</th></tr>
	</thead>		  
<% Else%>
    ...
    <tbody>
		...
		<% 'adesso per ogni messaggio guardo il post (topic) a cui si riferisce
		   ...
		   i=i+1
		     QuerySQL1="SELECT * "&_
" FROM FORUM_MESSAGES " &_
" WHERE ID=" & rsTabellaLavagna("ThreadParent") &" and comments<>'InizializzaDB' and Id_Social=1" &_
" ORDER BY ID desc;"
'Set objFSO = CreateObject("Scripting.FileSystemObject")
...
Set rsTabella1 = ConnessioneDB2.Execute(QuerySQL1) 
	...
	QueryCat = "SELECT Descrizione FROM CAT_CAT WHERE Id_Categoria = '"&rsTabellaLavagna("Id_Categoria")&"';"
		...
		categorianome = rsTabellaCat(0)
            ...
            <tr><td><a title="Visualizza Post di apertura discussione" href="../cSocial/ShowMessage.asp?scegli=1&amp;ID=<%=rsTabellaLavagna("ThreadParent")%>&amp;id_classe=<%=id_classe%>&amp;divid=<%=divid2%>&id_categoria=<%=rsTabellaLavagna("Id_Categoria")%>&categoria=<%=categorianome%>"><%=rsTabella1("Topic")%></a></td>

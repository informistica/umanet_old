...
" WHERE CodiceAllievo='" & cod & "' and Id_Classe='" &id_classe & "'  and ParentMessage=0 and comments<>'InizializzaDB' and Id_Social=3 " &_
...
<div class="box box-color box-bordered">
                                            <div class="box-title">
                                                <h3>
                                                    ...
                                                    Interrogazioni: Discussioni aperte (<%=num_post_totali%>) + Commenti(<%=num_messaggi%>) = Punti(<%=num_post_totali_punti+num_messaggi_punti%>)
                                                </h3>
                                            </div>
...
QuerySQL="SELECT * " &_
" FROM FORUM_MESSAGES " &_
" WHERE CodiceAllievo='" & cod & "' and Id_Classe='" &id_classe & "' and comments<>'InizializzaDB' and Id_Social=3 "&_
" and (DatePosted>= CONVERT(DATETIME,'" &DataClaq  &"', 104))" &_
...
<%If rsTabellaLavagna.BOF=True And rsTabellaLavagna.EOF=True Then %>
    ...
    <thead>
			  <tr><th colspan="5" align="center">  Non ci sono interrogazioni </th></tr>
	</thead>
<% Else%>
    ...
    <tbody>
		...
		<% 'adesso per ogni messaggio guardo il post (topic) a cui si riferisce
		   ...
		   i=i+1
		     QuerySQL1="SELECT * "&_
" FROM FORUM_MESSAGES " &_
" WHERE ID=" & rsTabellaLavagna("ThreadParent") &" and comments<>'InizializzaDB' and Id_Social=3" &_
...
Set rsTabella1 = ConnessioneDB3.Execute(QuerySQL1)
	'num_visualizzazioni=rsTabella1(0)
            ...
            <tr><td><a title="Visualizza Post di apertura interrogazione" href="../cSocial/ShowMessage.asp?scegli=3&amp;ID=<%=rsTabellaLavagna("ThreadParent")%>&amp;id_classe=<%=id_classe%>&amp;divid=<%=divid2%>&id_categoria<%=rsTabellaDiario("Id_Categoria")%>&categoria=<%=categorianome%>"><%=rsTabella1("Topic")%></a></td>

                <td><a title="Visualizza il messaggio nella discussione"    href="../cSocial/ShowMessage.asp?scegli=3&amp;ID=<%=rsTabellaLavagna("ID")%>&amp;id_classe=<%=id_classe%>&amp;divid=<%=divid2%>&id_categoria<%=rsTabellaDiario("Id_Categoria")%>&categoria=<%=categorianome%>"><%=rsTabellaLavagna("Topic")%></a></td>

...
function classereport(idp,DataCla,DataCla2)
	...
	response.write("id_classe-classe:"&id_classe&"-"&classe)
 	QuerySQL="Select count(*) from Allievi where Id_Classe='" & Session("Id_Classe")&"' and Attivo=1;"
	...
	QuerySQL="Select * from [3PERIODI] where Id_Classe='" & Session("Id_Classe")&"' order by Data;"
	Set rsTabellaPeriodi = ConnessioneDB.Execute(QuerySQL)
	
	' cancello tutti i dati precedenti 
	QuerySQL="delete from Report where Id_Classe='" & Session("Id_Classe")&"';"
	ConnessioneDB.Execute(QuerySQL)
 
	data_from=inizio_anno
	nome_file="report&"&classe&"_tutto.xls"
...
url=Server.MapPath(homesito & "/Grafici")& "/"&nome_file
url=Replace(url,"\","/")
...
objCreatedFile.WriteLine(intestazione)
	...
	QuerySQL="Select * from [Allievi] where Id_Classe='" & Session("Id_Classe")&"' and Attivo=1 order by Cognome, Nome;"
	Set rsTabellaAllievi = ConnessioneDB.Execute(QuerySQL)
	riga="<table><thead><tr><th><b>Cognome Nome</b></th>"
	for j=0 to numperiodi-1
		riga=riga+"<th><b>N.</b></th><th><center><b>VV</b></center></th><th><center><b>PT</b></center></th>"
	next
	riga=riga&"<th>M</th></tr></thead><tbody>"
	...
	do while not rsTabellaAllievi.eof
		...
		media=0
		if j=0 then
		  riga="<tr><th><b>"&rsTabellaAllievi("Cognome")&" "&rsTabellaAllievi("Nome")&"</b></th>"
		  j=1
		end if
		...
		do while not rsTabellaReportAllievo.eof
			...
			media=media+round(VV,1)
			PT=rsTabellaReportAllievo("PT")
			riga=riga&"<td> "&N&" </td><td><center> "&VV&" </center></td><td><center> "&PT&" </center></td>"
			rsTabellaReportAllievo.movenext
		   j=j+1
		loop
        media=media/j
        media=round(media,1)
		riga=riga&"<td>"&media&"</td></tr>"
		response.write(riga)
		objCreatedFile.WriteLine(riga)

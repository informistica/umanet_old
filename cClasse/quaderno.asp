...
%>
				...
				<!-- #include file = "../include/navigation_small.asp" -->
                     ...
                     <div class="box-title">
				      ...
				      <h3> <a name="#top"><i class="icon-folder-open"></i> </a><b>Quaderno <%=quaderno%> di <%=ReplaceCar(cognome)%>  &nbsp;<%=ReplaceCar(left(nome,1)&".") %></h3>&nbsp;</b>
								 ...
								 <% end if
								  ...
								  %>
                                    ...
                                    <ul class="dropdown-menu">
									  ...
									  <li><a target="_blank" href="../cFrasi/2compilaprefrase_recupero.asp?CodiceAllievo=<%=CodiceAllievo%>&cartella=<%=cartella%>">Piano di Recupero</a></li>
                                      <!--
...
<%QuerySQL="SELECT * FROM Classi WHERE Id_Classe='"&id_classe&"'"
   ...
   <div class="accordion" id="accordionMsg2">
    ...
    <!--<p> <input type="checkbox"  name="cbEmail" title="Selezionare per inviare un email allo studente">   Notifica per email &nbsp;&nbsp;&nbsp;<br>-->
     ...
     <br>
    <!-- <a href="aggiorna_messaggio.asp> </a>-->
    </form>
  ...
  <hr>
   <div class="box-title">
				        <h3> <a name="#"><i class="icon-reorder"></i></a>  Compiti <%=quaderno%><small title="Punti totalizzati"> (Pt.)</small></h3>
			          </div>
 <div class="row-fluid">
	...
	// alert (DataClaq);
	 // alert (DataClaq2);
		...
		if (elements["cbPS"].checked == true)
		   document.dati.action = "?umanet=<%=umanet%>&divid=<%=Session("divid")%>&classe=<%=classe%>&id_classe=<%=id_classe%>&PS=1&cod=<%=cod%>&DataClaq=" +DataClaq+ "&DataClaq2="+ DataClaq2 +"&daForm=1";
		 else
		   document.dati.action = "?umanet=<%=umanet%>&divid=<%=Session("divid")%>&classe=<%=classe%>&id_classe=<%=id_classe%>&PS=0&cod=<%=cod%>&DataClaq=" +DataClaq+ "&DataClaq2="+ DataClaq2 +"&daForm=1";
...
function caricacompiti(){
	...
	//alert(query);
	var url = "https://www.umanetexpo.net/expo2015Server/UECDL/script/cClasse/compiti_new.asp?"+query;
	//alert(url);
...
*/
 ...
 function aggiorna_studente(){
	...
	window.location.href = "quaderno.asp?umanet=<%=umanet%>&daStud=1&DataClaq=<%=DataCla%>&DataClaq2=<%=DataCla2%>&id_classe=<%=id_classe%>&classe=<%=classe%>&cod="+stud;

	...
	<div class="container-fluid" id="content">
	  ...
	  <div class="container-fluid">
				<div class="page-header">               
					<div class="pull-left">
						<h1> <i class="glyphicon-snowflake"></i>Inserisci compito</h1> 
...
QuerySQL="Select count(*) from preNodi where Id_Paragrafo='"&Paragrafo&"';"
set rsTabella=ConnessioneDB.Execute (QuerySQL) 
if rsTabella(0)>0 then
	QuerySQL="Select max(Posizione) from preNodi where Id_Paragrafo='"&Paragrafo&"';"  
	set rsTabella=ConnessioneDB.Execute (QuerySQL) 
	contPos=rsTabella(0)
else
	 contPos=0
...
if  txtDomande="" then
		...
		'QuerySQL="INSERT INTO preDomanda (Id_Mod, Id_Paragrafo,Quesito,Eseguita) SELECT '" & Modulo & "','" & Paragrafo "','" & Domanda & "'," & 0 & "';"
				if Scadenza <>"" and not (strcomp(Scadenza,"gg/mm/aaaa")=0) then
				   QuerySQL="  INSERT INTO preNodi (Id_Mod, Id_Paragrafo,Quesito,Eseguita,Posizione,Scadenza,Img,Files,Id_Sottoparagrafo)  SELECT '" & Modulo & "','" & Paragrafo & "', '" & Domanda & "'," & 0 & "," & contPos+k & ",'" & Scadenza & "'," & img & "'," & cFile & ",'" &CodiceSottopar & "';"
				   else
				   QuerySQL="  INSERT INTO preNodi (Id_Mod, Id_Paragrafo,Quesito,Eseguita,Posizione,Img,Files,Id_Sottoparagrafo)  SELECT '" & Modulo & "','" & Paragrafo & "', '" & Domanda & "'," & 0 & "," & contPos+k & "," & img & "," & cFile & ",'" &CodiceSottopar & "';"
				   end if
...
Commentatore= Session("Cognome") & " " & left(Session("Nome"),1) &"."   
 ...
 '  ConnessioneDB.Execute (QuerySQL) 
	...
	<%
		Response.Write "<h4>Inserimento avvenuto! </h4>"
	Else
	%>
...
<h5><a href="../home_app.asp?id_classe=<%=Session("Id_Classe")%>"> Torna al Libro </a></h5> 
 -->
 <br><br><br>

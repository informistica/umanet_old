 ...
 if Session("CaricatoFile")=true then ' viene settato da db-file-to-disk è serve per vedere 
	...
	if bAddNew then
	 ...
	 if Request("cbZip")<>"" then
	  ...
	  sSQL = "INSERT INTO FORUM_MESSAGES (AUTHORNAME,CODICEALLIEVO,ID_CLASSE,TOPIC,URLIMG,URLFILE,BACHECA,COMMENTS,Privato,Zip,Id_Social,DATEPOSTED,Id_Categoria,Abstract) VALUES (" & sName & scodAllievo & sIdClasse & sTopic & sUrlimg & sUrlfile & sBacheca & sComments & ","&privato&","&Zip&","&scegli&",'"&now&"',"&id_categoria&",'"& sAbstract&"');"
	  ...
	  sSQL1=sSQL
		...
		'		Set objFSO = CreateObject("Scripting.FileSystemObject")
				'url="DBQ=D:/inetpub/vhosts/umanet.net/httpdocs/anno_2013-2014/log_calcola.txt"
					url="C:\Inetpub\umanetroot\expo2015Server\logAbstract.txt"
		'		Set objCreatedFile = objFSO.CreateTextFile(url, True)
		'		objCreatedFile.WriteLine(request.form("Topic") & "<br>"&request("Breve") & "<br>"& sSQL)
...
'		end if
		...
		sSQL = "INSERT INTO FORUM_MESSAGES (PARENTMESSAGE,THREADPARENT,AUTHORNAME,CODICEALLIEVO,ID_CLASSE,TOPIC,URLIMG,URLFILE,BACHECA,COMMENTS,Id_Social,DatePosted,Id_Categoria,Abstract) VALUES (" & iParent & "," &  iThread & "," & sName & scodAllievo & sIdClasse &  sTopic & sUrlimg & sUrlfile & sBacheca & sComments & ","&scegli&",'"&now&"',"&id_categoria&",'"& sAbstract&"');"
...
if cbEmail2<>"" then
	 '  response.write("Email alla classe")
        QuerySQL="Select CodiceAllievo,Email from Allievi where Id_Classe='"&id_classe&"' and Email<>'' ;"
		...
		'Da completare, devo per ogni codiceallievo della discussione leggere la sua email ed inviare		 
			...
			sSQL="SELECT distinct(FORUM_MESSAGES.CodiceAllievo), Allievi.Email, FORUM_MESSAGES.ThreadParent, Allievi.Id_Classe " &_
" FROM Allievi INNER JOIN FORUM_MESSAGES ON Allievi.CodiceAllievo = FORUM_MESSAGES.CodiceAllievo " &_
" WHERE FORUM_MESSAGES.ID="&Request("ParentId")&";"	
	...
	'	response.write("<br>VERO1")
		...
		sSQL="SELECT distinct(FORUM_MESSAGES.CodiceAllievo), Allievi.Email, FORUM_MESSAGES.ThreadParent, Allievi.Id_Classe " &_
" FROM Allievi INNER JOIN FORUM_MESSAGES ON Allievi.CodiceAllievo = FORUM_MESSAGES.CodiceAllievo " &_
" WHERE FORUM_MESSAGES.ThreadParent="&iParent&";"	
 set rsTabella = conn.execute (sSQL)   
 ...
 if (cbEmail0<>"") or (cbEmail1<>"") or (cbEmail2<>"") then  
  do while not rsTabella.eof
    ...
    linkAvviso=dominio&homesito&"/script/cSocial/ShowMessage.asp?scegli="&scegli&"&ID="&maxID&"&RCount=0&TParent="&maxID&"&id_classe="&Id_Classe&"&Classe="&Session("Cartella")&"&Cartella="&Session("Cartella")&"&CodiceAllievo="&rsTabella("CodiceAllievo")&"&by_email=1&DB="&Session("DB")&"&id_materia="&Session("Id_Materia")&"&materia="&Session("Materia")&"&id_categoria="&Session("id_categoria")&"&categoria="&Session("categoria")
   ...
   loop
	...
	sBody="Messaggio da: " & ucase(session("social")) &":"& messaggioda & "<br>Autore: "&request("AuthorName") &"<br><br>" & Request("Message")
	    ...
	    ' response.write("Email prof")
		  linkAvviso=dominio&homesito&"/script/cSocial/ShowMessage.asp?scegli="&scegli&"&ID="&maxID&"&RCount=0&TParent="&maxID&"&divid="&divid&"&id_classe="&Id_Classe&"&Classe="&Session("Cartella")&"&CodiceAllievo="&codAdmin&"&by_email=1&DB="&Session("DB")&"&id_materia="&Session("Id_Materia")&"&materia="&Session("Materia")&"&id_categoria="&Session("id_categoria")&"&categoria="&Session("categoria")&"&Cartella="&Session("Cartella")
		sBody = sBody  &"  <br> <a title='Vai ad Umanet' href='"& linkAvviso&"'>Entra in Umanet Evolution 3.0</a> <img alt='enlightened' height='20' src='https://www.umanetexpo.net/expo2015Server/UECDL/js/plugins/ckeditor/plugins/smiley/images/lightbulb.gif' title='Idee per evolvere' width='20' /> "
		'sBody = Server.HTMLEncode(sBody)
	   sTo=eMailAdmin
	   'sTo="mauro.spinarelli@gmail.com"
...
<FORM ACTION = "PreviewMessage.asp?cognome=<%=request.QueryString("cognome")%>&nome=<%=request.QueryString("nome")%>&Zip=<%=Session("Zip")%>&scegli=<%=scegli%>&cbEmail=<%=cbEmail%>&cbEmailProf=<%=cbEmailProf%>&TParent=<%=ParentID%>&RCount=<%=RCount%>&txtNUMREC=<%=Request.form("txtNUMREC")%>" METHOD = "POST">

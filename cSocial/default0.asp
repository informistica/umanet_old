...
<FORM class='form-horizontal form-striped' ACTION = 'forum_search.asp?scegli=<%=scegli%>&divid=<%=divid%>&id_classe=<%=id_classe%>&cartella=<%=cartella%>&bacheca=<%=bacheca%>'><b> Cerca <img src="img/icon_aim.gif"></b>
...
' rs1.Open sSQLC, conn, 1,3
 ...
 rs1.close
   ...
   if (Session("Admin")=true) or (rs("Visibile")<>0) then
	 ...
	 response.write " <TD>"
	 if rs("Anonimo")=1 then
	 	response.write "<A HREF = '#'>Anonimo </A>"
	 else	
 ...
 response.write "<tr class="&classe_riga&"> <TD><A HREF='default0.asp?nome=" & request.QueryString("nome") &"&cognome="&request.QueryString("cognome")&"&categoria="&rtrim(rs(2))&"&id_categoria="&rs(0)&"&id_classe=" & id_classe & "&cartella="&cartella&"&scegli="&scegli&"&bacheca="&bacheca&"'>"&lCtr&"</A></TD>"
    ...
    "<a href='#modal-1' onClick='modifica("&rs(0)&","""&rs(2)&""")' data-toggle='modal'><i style='text-decoration:none' class='icon-pencil' title='Modifica categoria'></i></a>"&_
    "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"&_
    "<a href='#modal-2' onClick='elimina("&rs(0)&","""&rs(2)&""")' data-toggle='modal'><i style='text-decoration:none' class='icon-trash' title='Elimina categoria'></i></a>"&_
    "</center></TD></TR>"
		else
		response.write "</TR>"
		end if
...
%>
    ...
    <form name="eli" id="eli" action="elimina_categoria.asp" method="post">
		...
		function modifica(id,categoria){
		 //alert(categoria);

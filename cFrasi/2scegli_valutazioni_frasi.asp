...
'---------
 ...
 %>
	...
	<div class="container-fluid" id="content">
	  ...
	  <div class="container-fluid">
				<div class="page-header">
					<div class="pull-left">
						<h1> <i class="icon-reply"></i> Valuta frasi</h1>
	...
	QuerySQL="SELECT * " &_
"FROM preFrasi WHERE preFrasi.Id_Paragrafo='" & CodiceTest & "' and Id_Mod='"&ID_MOD & "' and Id_Sottoparagrafo='"&CodiceSottopar&"' order by Posizione"
	else
'	QuerySQL="SELECT * " &_
'"FROM preFrasi WHERE  (Id_Mod='"&ID_MOD & "' or Id_paragrafo='"&Paragrafo&"') or Id_Sottoparagrafo='"&CodiceSottopar&"' and Img=1"
'	QuerySQL="SELECT * " &_
'"FROM preFrasi WHERE  (Id_Mod='"&ID_MOD & "' and Id_paragrafo='"&Paragrafo&"')  and Img=1"
QuerySQL="SELECT * " &_
"FROM preFrasi WHERE  Id_Mod='"&ID_MOD&"'  and Img=1 order by Posizione"
	end if
...
QuerySQLInt="SELECT * FROM FORUM_MESSAGES WHERE Topic='"&Capitolo & "' and Id_Classe ='" & Session("Id_Classe") &"'"
Set rsTabellaInterroga = ConnessioneDB.Execute(QuerySQLInt)
id_categoria=rsTabellaInterroga("Id_Categoria")
id_classe=Session("Id_Classe")
parentmessage=rsTabellaInterroga("ID")
	...
	'if (i=0) or (StrComp(capitolo, rsTabella(0)) <> 0) then'
	 ...
	 %>
			  <% if rsTabella("img")=1  then
					...
					<a href="2inserisci_valutazioni_frasi.asp?BoxApro=<%=BoxApro%>&Tipo=0&FraseScelta=<%=rsTabella.fields("Quesito")%>&Cartella=<%=Cartella%>&Capitolo=<%=Capitolo%>&TitoloParagrafo=<%=TitoloParagrafo%>&Paragrafo=<%=Paragrafo%>&Modulo=<%=Modulo%>&CodiceTest=<%=CodiceTest%>&prefrase=1&idprefrase=<%=rsTabella("Id_prefrase")%>&CodiceSottopar=<%=CodiceSottopar%>"><%=image%>&nbsp;<%=server.htmlencode(rsTabella.fields("Quesito"))%>
					</a> </li>

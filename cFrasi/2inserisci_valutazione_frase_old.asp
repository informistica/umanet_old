...
<%  if (session("CodiceAllievo")="") or (session("Id_Classe")="") then %>
  ...
  <% else %>
        ...
        <!--<body>-->
        <body onLoad="getElement();" class='theme-<%=session("stile")%>' >

                <%else%>
        <body onLoad="getElement1();" class='theme-<%=session("stile")%>' >
        <%end if%>
  <% end if %>
...
'
  ...
  if (CIAbilitato=0)  and Session("Admin")=False then  %>
  <script>
		 document.write('<p><textarea class=\"input-block-level\" rows=<%=righe%> id=\"txtS1\" <% if dis = true then response.write("disabled") end if %> tabindex=\"<%=(2*i)+1%>\" name=\"txtSpiegazione\" onkeydown = \"onKeyDown()\" ></textarea></p>');
		</script>
		<noscript>Abilita JavaScript per visualizzare correttamente il sito.</noscript>

		<script>
		var x = "<% Response.write(ReplaceCar(sReadAll1)) %>";
		document.getElementById('txtS1').innerHTML = x;
		</script>

		<%else%>
		 <p><textarea class="input-block-level" rows="<%=righe%>" name="txtSpiegazione" <% if dis = true then response.write("disabled") end if %> ><%=Response.write(ReplaceCar(sReadAll1))%></textarea></p>
...
if rsTabellaNew("Img")=1 then%>
 ...
 <%     QuerySQL1="Select * from Frasi_Img where Id_Frase="& rsTabellaNew("CodiceFrase")&";"
      ...
      <% nome=right(rsTabella1("Nome"),len(rsTabella1("Nome"))- instr(ucase(rsTabella1("Nome")),"C:\FAKEPATH\"))
       ...
       <p align="center">
     <%  gdoc="false"
	     if ((instr(rsTabella1("Url"),"docs.google.com")<>0) or (instr(rsTabella1("Url"),"drive.google.com")<>0)  )  then ' nb è voluto il tp:// invece di https:// perchp altrimenti essendo all'inizio restituisce 0 che è come se non fosse presente
         gdoc="true"
...
<%if (session("Admin")=true) then %>
 <p><input class="input-mini" type="text" name="txtVAL" value="<%=rsTabellaNew.Fields("Voto")%>" size="1"  ><b>
	Valutazione </b>
  <%end if%>
   <br><br>
   <%
...
<% if rsTabellaNew("Img")=1 then %>
<input type="button" id="btnImg" value="Invia" name="B1" class="btn"> </p>
<%else%>
 <input onclick="getParametri(0)" type="button" id="btnNoImg" value="Invia" name="B1" class="btn"> </p>
 <%end if%>
        ...
        <script>
			  	l= "<%=Request.ServerVariables("PATH_INFO")%>"
				</script>
<script src="parametri.js"></script>

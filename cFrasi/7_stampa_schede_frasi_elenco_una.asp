...
'
 ...
 QuerySQL="SELECT * FROM MODULO_PARAGRAFO_FRASI1 Where Id_Prefrase='"&FraseScelta&"';"
 'QuerySQL=Request.QueryString("QuerySQL")
 'response.write("<br>1"&Request.Form("txtSQL") )
 'response.write("<br>2"&QuerySQL)
'
...
' controllo se devo fare uridecode
'  Dim objScript
'     Set objScript = Server.CreateObject("ScriptControl")
'     objScript.Language = "JavaScript"
'     URLDecode = objScript.Eval("decodeURIComponent(""" & sReadAll & """.replace(/\+/g,"" ""))")
'     sReadAll=URLDecode

s = Split(sReadAll, "%")
response.write("UBound(s)="&UBound(s))
if UBound(s)>10 then' è codificato
else
...
%>
 ...
 <% if supersint="" then%>
<ul>
<li> <b><%=server.htmlencode(rsTabella(3))%> </b> (<%=response.write(rsTabella("Cognome")&" "&left(rsTabella("Nome"),1)&".")%>)</li>
...
<%=sReadAll0%>
		 ...
		 <%     QuerySQL1="Select * from Frasi_Img where Id_Frase="& rsTabella("CodiceFrase")&";"

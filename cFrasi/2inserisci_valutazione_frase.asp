...
QuerySQLEXT = "Select Estesa from PreFrasi where ID_prefrase = "&Id_prefrase&""

Set rsTabellaP = ConnessioneDB.Execute(QuerySQLEXT)
	estesa=rsTabellaP("Estesa")
   if strcomp(estesa,"True")=1 then
     ext=1
   else
 	ext=0
   end if
				...
				<div class="row-fluid">
				  <div class="span12">
				    <div class="box">
					 ...
					 <% 
					' response.write(QuerySQLEXT &" " &ext & " " &estesa)
					 if ext=1 then ' carico testo esteso 
					   Set objFSO = CreateObject("Scripting.FileSystemObject") 
					 '  Const ForReading = 1, ForWriting = 2, ForAppending = 8
					   url=Server.MapPath(homesito)& "/Db"&Session("DB")&"/Materie/"&Session("ID_Materia")& "/" &cartella &"/" &ID_MOD&"_Esercizi/"&Codice_Test&"_"&Id_Prefrase&".txt"
					   	url=Replace(url,"\","/")
						   'response.write(url)
					    if objFSO.FileExists(url) then
					 	Set objTextFile = objFSO.OpenTextFile(url, ForReading)
					 	sReadAllTesto="" 'pulisco sReadAll -> altrimenti rimane la vecchia spiegazione
					 	sReadAllTesto = objTextFile.ReadAll
					 	'sReadAll=url
					 	objTextFile.Close
					 	else
					 	 response.write("Il file non esiste:"&url)
					 	 sReadAllTesto="Il file " &url &" non esiste"
				    	end if
					   end if %>
...
' stampo il testo dell'esercizio
	response.write(sReadAllTesto)
  	%>

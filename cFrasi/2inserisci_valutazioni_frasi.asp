...
'response.write(rsTabellaNew.Fields("Segnalata")&"<br>")
	...
	if (rsTabellaNew.Fields("Segnalata")=1) or (rsTabellaNew.Fields("Segnalata")=2)   then
		...
		segnalazioni=segnalazioni&"<tr><td>"&rsTabellaNew("Voto")&"</td><td>" &rsTabellaNew("Cognome")&" "&left(rsTabellaNew("Nome"),1)&".</td> "
		f="<span style='color:red'>Segnalata</span>"
		Else
		segnalati_pos=segnalati_pos&"'"&rsTabellaNew("CodiceAllievo")&"'"&","
		segnalazioni_pos=segnalazioni_pos&"<tr><td>"&rsTabellaNew("Voto")&"</td><td>" &rsTabellaNew("Cognome")&" "&left(rsTabellaNew("Nome"),1)&".</td> "
	   ...
	   'response.write("<br>Segnalata:"&rsTabellaNew.Fields("CodiceFrase"))
		...
		if objFSO.FileExists(url_feedback) then
			...
			feedback=ReplaceCar(feedback)
			if (rsTabellaNew.Fields("Segnalata")=1) then
			segnalazioni=segnalazioni&"<td>"&feedback&"</td></tr>"
			end if
			if (rsTabellaNew.Fields("Segnalata")=2) then
			segnalazioni_pos=segnalazioni_pos&"<td>"&feedback&"</td></tr>"
			end if
...
if rsTabellaNew("Img")=1 then%>
 ...
 <%     QuerySQL1="Select * from Frasi_Img where Id_Frase="& rsTabellaNew("CodiceFrase")&";"
	 ...
	 <% else%>
		   ...
		   <%else
				if (instr(rsTabella1("Url"),"tp://")<>0) or (instr(rsTabella1("Url"),"tps://")<>0) and ((instr(rsTabella1("Url"),".htm")<>0) or (instr(rsTabella1("Url"),".html")<>0) or (instr(rsTabella1("Url"),".php")<>0)or (instr(rsTabella1("Url"),"jsbin.com")<>0) )  then ' nb è voluto il tp:// invece di https:// perchp altrimenti essendo all'inizio restituisce 0 che è come se non fosse presente
					pagina=1
					flag=2
...
tabella="<table class='table table-hover table-nomargin table-bordered dataTable dataTable-noheader dataTable-nofooter'><thead><tr><th class='hidden-480'>Punti</th><th class='hidden-480'>Studente</th><th class='hidden-480'>Feedback</th></tr></thead><tbody>"
 if segnalati="" then
...
'   rsTabellaNC.movenext
'   loop
	segnalazioni=tabella&segnalazioni&"</table>"
...
'   'response.write(q)
   response.write("<br><span style='color:green'><b>Segnalazioni(+):</span></b>")
	segnalazioni_pos=tabella&segnalazioni_pos&"</table>"

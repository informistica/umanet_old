...
Conn1.Provider = "Microsoft.Jet.OLEDB.4.0"
 ...
 if daForum<>"" then
	     Conn1.open "Data Source=" & Server.MapPath(homesito)& "/database/" & Session("DBForum")    
		  ...
		  else 'if daLavagna<>"" then
	              Conn1.open "Data Source=" & Server.MapPath(homesito)& "/database/" & Session("DBLavagna")  
		       'else
			      'Conn1.open "Data Source=" & Server.MapPath(homesito)& "/database/" & Session("DBCopiatestonline")
			   'end if
		  end if
	end if
  ...
  'DestinationFileName = DestinationPath & "\" & Form("SourceFile").FileName
   if daForum<>"" then
	      DestinationPath = Server.MapPath(homesito)& "/Materie/"&Session("ID_Materia")&"/" & Session("cartella") &"/file_forum"   
			 ...
			 '   else  if daFrase<>"" then
			  '       DestinationPath=Server.MapPath(homesito)&"/Materie/"&Session("ID_Materia")& "/" & Session("cartella") &"/" &Modulo&"_Frasi/"    
	             '   end if
		       ' end if
		   end if
	end if
...
'  RS.Update
	...
	'....
			...
			session("File")=DestinationFileName
...
'				objCreatedFile.Close
 ...
 ' response.write "<br>See ListFiles table in " & Server.MapPath(homesito)& "/database/upload.mdb"   & " database.</Font>"
	...
	Session("CaricatoFile")=true
	Session("CaricatoFile2")=true
	if daFrase<>"" then
	'......
	  Session("NomeFileForum")=Form("SourceFile").FileName
	'else
...
Function GetConnection()
 ...
 'Conn.open "Data Source=" & Server.MapPath("upload.mdb") 
  ...
  if daForum<>"" then
	...
	if daDiario<>"" then
	    Conn.open "Data Source=" & Server.MapPath(homesito)& "/database/" & Session("DBDiario")  
    else
	  ' if daLavagna<>"" then
	      Conn.open "Data Source=" & Server.MapPath(homesito)& "/database/" & Session("DBLavagna")  
	  ' else
	  '     if daFrase<>"" then
		'      Conn.open "Data Source=" & Server.MapPath(homesito)& "/database/" & Session("DBCopiatestonline") 
		'   end if
	   'end if
	end if
  end if
...
<tr><td>&nbsp;</td><td>ooooo<% response.write(DestinationFileName)%>
</td></tr>
